...
class Env {
    ...
    val architecturyVersions = mapOf(
        "1.20.1" to "47.3.5"
    )
    ...
    fun atLeast(version0: String,version: String) = stonecutter.compare(version0, version) >= 0
    fun atMost(version0: String,version: String) = stonecutter.compare(version0, version) <= 0
...
val apis = arrayListOf(
    APISource(
        DepType.API, APIModInfo(
            if (env.atMost("1.16.5")) {
                "fabric"
            } else {
                "fabric-api"
            }, "fabric-api"
        ), "net.fabricmc.fabric-api:fabric-api",
        if (property("loom.platform").toString() == "fabric") {
            Optional.of(
                optionalVersionProperty("deps.api.fabric").orElse(
                    VersionRange(
                        env.fabricVersions[env.mcVersion.min] ?: "", env.fabricVersions[env.mcVersion.min] ?: ""
                    )
                )
            )
        } else {
        ...
        DepType.FLETCHINGTABLE, APIModInfo("CSKdjzLF"),
        mavenLocation = "",
        versionRange = Optional.of(VersionRange("1.0.0-beta.3","")),
        enableCondition = {
                _ -> true
        }
    )
...
class SpecialMultiversionedConstants {
    ...
    val awField = if (env.isFabric) "  \"accessWidener\" : \"brick_lib_core.accesswidener\"," else ""
    ...
    apis.forEach { src ->
        if (src.enabled) {
            src.versionRange.ifPresent { ver ->
                ...
                if (src.type == DepType.FLETCHINGTABLE) {
                    src.modInfo.modid?.let {
                        val dependency = fletchingTable.modrinth(it, env.mcVersion.min, env.loader){
                            constraint.set { it0 -> env.atLeast(it0.version,src.versionRange.orElse(VersionRange("0","")).min) }
                        }
...
abstract class ProcessResourcesExtension : ProcessResources() {
    ...
    val autoPluralize = arrayListOf(
        ...
        "/data/brick_lib_core/loot_table",
        "/data/brick_lib_core/recipe",
        "/data/brick_lib_core/tags/item",
    )
...
fletchingTable {
    if (env.isForge || env.isNeo) {
        accessConverter.register(sourceSets.main) {
            add("brick_lib_core.accesswidener")
        }
    }
}
