...
public class ModLang {
    ...
    public static final String langPath = Constants.versionFolderPath() + "\\datagen";

    public static int generate() throws IOException {
        int count = 0;
        if(!Constants.isWorldInitiated()){
            BrickLib.LOGGER.error("World variables are not initialed");
            return 0;
        }
    ...
    private static String generateContent() {
        return new GsonBuilder().setPrettyPrinting().create().toJsonTree(languageKeys.stream()
                ...
                .collect(Collectors.toMap( k -> k, v -> "§cNO TRANSLATE"))).toString();
    }
